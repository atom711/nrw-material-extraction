...
## Objective & Purpose
This repository houses an automated Python processing pipeline designed to extract continuous complex relative permittivity (\(\epsilon^*r = \epsilon'_r - j\epsilon''_r\)) and absolute dielectric loss tangent (\(\tan \delta\)) from 2-port complex scattering parameters (\(S_{11}, S_{21}\)). 
...
2. **Mathematical Transformation Core:**
   ...
   * Solves for boundary criteria transitions: 
     \[\Gamma = X \pm \sqrt{X^2 - 1.0} \quad \text{where} \quad \vert{}\Gamma\vert{} \le 1.0\]
...
* `plots/nrw_parameter_extraction.png` - Extracted material parameters vs. target specification baselines.
...
![NRW Extraction Metrics](plots/nrw_parameter_extraction.png)
